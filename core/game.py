...
from items.items import (
    Potion, Weapon, Armor, OffHand, Chest, IndoorChest, OutdoorChest, LockedChest, Book, BOOK_LOAD_ERRORS, lesser_healing_potion, greater_healing_potion, wood_plank, meat, green_apple, fromage, 
...
class Game:
    ...
    def remove_item_from_player(self, item_id, count=1):
        ...
        for item in list(self.player.inventory.items):
            ...
            if item.name != template.name:
                continue
            # Take at most `remaining` units out of this stack -- a
            # stackable item (Food, Potion, ammunition) only loses part
            # of its count, and only drops its slot once it's empty.
            taken = min(remaining, item.count)
            item.count -= taken
            remaining -= taken
            if item.count <= 0:
    ...
    def _remove_entity_from_world(self, entity):
        ...
        for chunk in self.overworld_chunks.values():
            population = chunk.get("population")
            if population and entity in population:
    ...
    def recruit_companion(self, source_entity, escort_id=None, reward_consequences=None, escort_hours=0, dialogue=None):
        ...
        that fallback.
        """
        self._remove_entity_from_world(source_entity)
    ...
    def get_chest_at(self, x, y):
        ...
        for item in self.game_map.items_on_ground:
            if isinstance(item, Chest) and item.x == x and item.y == y:
                return item
            elif isinstance(item, IndoorChest) and item.x == x and item.y == y:
                return item
            elif isinstance(item, OutdoorChest) and item.x == x and item.y == y:
                return item
        return None
